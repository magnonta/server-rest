...
if [ "$OS" = "Darwin" ]; then
    ...
    echo "# Iniciar Docker (usar service se systemctl não funcionar no WSL2):"
    echo "sudo systemctl start docker 2>/dev/null || sudo service docker start"
    echo "sudo systemctl enable docker 2>/dev/null || true"
    echo "sudo usermod -aG docker \$USER"
    echo ""
    echo "# 3. Instalar kubectl:"
    echo "curl -LO \"https://dl.k8s.io/release/\$(curl -L -s https://dl.k8s.io/release/stable.txt)/bin/linux/\$(dpkg --print-architecture)/kubectl\""
    echo "sudo install -o root -g root -m 0755 kubectl /usr/local/bin/kubectl"
    echo ""
    echo "# 4. Instalar kind:"
    echo "curl -Lo ./kind \"https://kind.sigs.k8s.io/dl/v0.31.0/kind-linux-\$(dpkg --print-architecture)\""
    echo "chmod +x ./kind"
